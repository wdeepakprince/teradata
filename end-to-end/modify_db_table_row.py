...
# Create the SQL Query
tera_sql_query = f"""
CREATE MULTISET TABLE  {tera_db}.{tera_table} (
    ID INTEGER,
    EmpName VARCHAR(100),
    Salary DECIMAL(10, 2),
    JoiningDate DATE
...
# View all the created Table
# --------------------------

# Query the DBC.TablesV view to get a list of tables
# DBC.TablesV filters out tables you don't have access to.
# Create the SQL Query
tera_sql_query = f"""
    SELECT *
    FROM DBC.TablesV
...
try:
    cur.execute(tera_sql_query)
    print(f"Table viewed successfully!")
    print("\n")

    # Print all Tables available in cursor
    for i in cur:
        print(i[1]) # 0 - retrieves DB name; 1 - retrieves Table name
    print("\n")
    
except Exception as e:
    print("Table NOT viewed!")
    print("\n")
    print(f"An unexpected error occurred: {e}")
    print("\n")
